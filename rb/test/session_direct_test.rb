...
class SessionDirectTest < Minitest::Test
  def test_direct_list_session
    setup = session_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-session", setup[:live] ? "live" : "unit")
    if _should_skip
...
def session_direct_setup(mockres)
