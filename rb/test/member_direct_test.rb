...
class MemberDirectTest < Minitest::Test
  def test_direct_list_member
    setup = member_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-member", setup[:live] ? "live" : "unit")
    if _should_skip
...
def member_direct_setup(mockres)
