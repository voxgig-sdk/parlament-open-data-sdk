...
require_relative 'ParlamentOpenData_types'
...
class ParlamentOpenDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ParlamentOpenDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ParlamentOpenDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ParlamentOpenDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ParlamentOpenDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ParlamentOpenDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ParlamentOpenDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ParlamentOpenDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ParlamentOpenDataError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ParlamentOpenDataError => err
    ...
    if fetched.is_a?(Hash)
      status = ParlamentOpenDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ParlamentOpenDataError.new(
        "graphql_error", "ParlamentOpenDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ParlamentOpenDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
