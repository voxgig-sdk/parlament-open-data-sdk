...
class BusinessEntity
  ...
  def make
    opts = @_entopts.dup
    BusinessEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ParlamentOpenDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Business, Hash] the current Business data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ParlamentOpenDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Business fields)
  def match_get
  ...
  #
  # @param reqmatch [BusinessListMatch, Hash, nil] match filter (any subset of
  #   Business fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Business>, Array] the matching Business items; raises ParlamentOpenDataError on failure
