...
module ParlamentOpenDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ParlamentOpenDataResponse.new({})
    ...
    if fetch_err
      ...
      response = ParlamentOpenDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ParlamentOpenDataResponse.new(fetched)
    else
