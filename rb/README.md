...
The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Business` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
raise a `ParlamentOpenDataError` (a `StandardError` subclass), so wrap
