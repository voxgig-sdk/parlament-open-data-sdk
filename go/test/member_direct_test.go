...
type memberDirectSetupResult struct {
...
func memberDirectSetup(mockres any) *memberDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PARLAMENT_OPEN_DATA_TEST_MEMBER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PARLAMENT_OPEN_DATA_TEST_MEMBER_ENTID"]; ok {
		...
		return &memberDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &memberDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
