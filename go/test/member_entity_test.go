...
func TestMemberEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PARLAMENT_OPEN_DATA_TEST_MEMBER_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		_, memberRef01ListOk := memberRef01ListResult.([]any)
...
func memberBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PARLAMENT_OPEN_DATA_TEST_MEMBER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PARLAMENT_OPEN_DATA_TEST_MEMBER_ENTID"])
