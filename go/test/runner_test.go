...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PARLAMENTOPENDATA_TEST_LIVE") == "TRUE" ||
		os.Getenv("PARLAMENTOPENDATA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PARLAMENTOPENDATA_TEST_EXPLAIN"); explain != "" {
