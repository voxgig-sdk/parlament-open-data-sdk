...
type ParlamentOpenDataSDK struct {
	Mode     string
...
func (sdk *ParlamentOpenDataSDK) OptionsMap() map[string]any {
...
func (sdk *ParlamentOpenDataSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *ParlamentOpenDataSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *ParlamentOpenDataSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ParlamentOpenDataSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *ParlamentOpenDataSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *ParlamentOpenDataSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("ParlamentOpenDataSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *ParlamentOpenDataSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ParlamentOpenDataSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("ParlamentOpenDataSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *ParlamentOpenDataSDK {
	...
	sdk := NewParlamentOpenDataSDK(sdkopts)
	sdk.Mode = "test"
