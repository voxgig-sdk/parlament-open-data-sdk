...
// ParlamentOpenData SDK feature test
//
...
class FtCtrl extends ParlamentOpenDataControl
{
...
class FtClient
{
    ...
    private ParlamentOpenDataUtility $utility;

    public function __construct(ParlamentOpenDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): ParlamentOpenDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ParlamentOpenDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ParlamentOpenDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ParlamentOpenDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ParlamentOpenDataConfig::make_feature($name);
    ...
    private static function buildUrl(ParlamentOpenDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ParlamentOpenDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ParlamentOpenDataResponse($fetched);
        }
    ...
    private function fail(ParlamentOpenDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ParlamentOpenDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
