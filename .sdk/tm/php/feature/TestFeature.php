...
class ParlamentOpenDataTestFeature extends ParlamentOpenDataBaseFeature
{
    ...
    public function init(ParlamentOpenDataContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ParlamentOpenDataContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ParlamentOpenDataParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ParlamentOpenDataContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ParlamentOpenDataContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ParlamentOpenDataParam::call($ctx, $k);
