...
from projectname_sdk.core.control import ParlamentOpenDataControl
...
class ParlamentOpenDataContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = ParlamentOpenDataControl()
        ...
        if isinstance(sp, ParlamentOpenDataSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ParlamentOpenDataResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ParlamentOpenDataResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return ParlamentOpenDataOperation({})
    ...
    def make_error(self, code, msg):
        return ParlamentOpenDataError(code, msg, self)
