...
[MCP](https://modelcontextprotocol.io) server exposing the ParlamentOpenData SDK as
two agent tools — `parlament-open-data_list` and `parlament-open-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/parlament-open-data-mcp)
make build

# 2. Provide credentials via the environment
export PARLAMENT_OPEN_DATA_APIKEY=sk_live_xxx
...
```jsonc
// parlament-open-data_list: first page of records
{ "entity": "business" }
{ "entity": "business", "query": { } }

// parlament-open-data_load: one record by id
{ "entity": "business", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/parlament-open-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user parlament-open-data \
     -- "$PWD"/dist/*/parlament-open-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `parlament-open-data_list` and `parlament-open-data_load` tools now appear
   in new sessions. Ask the agent to *"list business using parlament-open-data"*
   and it calls `parlament-open-data_list` with `{"entity":"business"}`.
...
```sh
export PARLAMENT_OPEN_DATA_APIKEY=sk_live_xxx            # API key
export PARLAMENT_OPEN_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `parlament-open-data_list` tool
...
### Call the `parlament-open-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `parlament-open-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `parlament-open-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PARLAMENT_OPEN_DATA_APIKEY` | API key sent with every request. |
| `PARLAMENT_OPEN_DATA_BASE` | Optional override of the API base URL. |
