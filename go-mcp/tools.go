...
type Args struct {
	Entity string         `json:"entity" jsonschema:"business | member | session"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ParlamentOpenDataSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "parlament-open-data_list",
		Description: "List records from ParlamentOpenData. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "parlament-open-data_load",
		Description: "Load a single record from ParlamentOpenData. " +
...
func runOp(client *sdk.ParlamentOpenDataSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ParlamentOpenDataSDK, name string) (sdk.ParlamentOpenDataEntity, error) {
	switch strings.ToLower(name) {
	case "business":
		return client.Business(nil), nil
	case "member":
