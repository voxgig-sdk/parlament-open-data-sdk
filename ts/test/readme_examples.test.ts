...
//     (\`new ParlamentOpenDataSDK(...)\` / \`ParlamentOpenDataSDK.test(...)\` become
//     \`ParlamentOpenDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ParlamentOpenDataSDK'
...
const TEST_SEED = {"entity":{"business":{"test01":{"id":"test01"}},"member":{"test01":{"id":"test01"}},"session":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ParlamentOpenDataSDK(...)\` and any
// \`ParlamentOpenDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ParlamentOpenDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ParlamentOpenDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
