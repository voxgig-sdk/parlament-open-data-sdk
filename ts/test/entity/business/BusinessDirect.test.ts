...
describe('BusinessDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new ParlamentOpenDataSDK({
  ...
  test('direct-list-business', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-business', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PARLAMENT_OPEN_DATA_TEST_BUSINESS_ENTID']
