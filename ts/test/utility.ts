...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.PARLAMENTOPENDATA_TEST_LIVE ||
    'TRUE' === process.env.PARLAMENTOPENDATA_TEST_OVERRIDE
  ) {
  ...
  m.PARLAMENTOPENDATA_TEST_EXPLAIN = process.env.PARLAMENTOPENDATA_TEST_EXPLAIN || m.PARLAMENTOPENDATA_TEST_EXPLAIN
