...
class ParlamentOpenDataSDK {
  ...
  async direct(fetchargs?: any) {
    if (!this._options.allow.op.includes('direct')) {
      return {
        ok: false,
        err: new Error('ParlamentOpenDataSDK: direct: operation not allowed by' +
  ...
  async graphql(query: string, variables?: any, ctrl?: any) {
    ...
    if (!options.allow.op.includes('graphql')) {
      return {
        ok: false,
        err: new Error('ParlamentOpenDataSDK: graphql: operation not allowed by' +
    ...
    if (null != errors && Array.isArray(errors) && 0 < errors.length) {
      ...
      const err: any = new Error('ParlamentOpenDataSDK: graphql: ' +
  ...
  Business(entopts?: Record<string, any>) {
    const self = this
    return new BusinessEntity(self, entopts)
  }
  ...
  static test(testoptsarg?: any, sdkoptsarg?: any) {
    ...
    const testsdk = new ParlamentOpenDataSDK(sdkopts)
    testsdk._mode = 'test'
  ...
  tester(testopts?: any, sdkopts?: any) {
    return ParlamentOpenDataSDK.test(testopts, sdkopts)
  }


  toJSON() {
    return { name: 'ParlamentOpenData' }
  }

  toString() {
    return 'ParlamentOpenData ' + this._utility.struct.jsonify(this.toJSON())
  }
...
const SDK = ParlamentOpenDataSDK
