...
from parlamentopendata_sdk.parlamentopendata_types import (
    Business,
    BusinessListMatch,
)
...
class BusinessEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BusinessEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Business]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Business().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
