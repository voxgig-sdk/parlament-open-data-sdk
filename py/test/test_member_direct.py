...
class TestMemberDirect:

    def test_should_direct_list_member(self):
        setup = _member_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-member", "live" if setup["live"] else "unit")
        if _skip:
...
def _member_direct_setup(mockres):
