...
class TestBusinessDirect:

    def test_should_direct_list_business(self):
        setup = _business_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-business", "live" if setup["live"] else "unit")
        if _skip:
...
def _business_direct_setup(mockres):
