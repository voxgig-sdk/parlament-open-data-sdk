    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PARLAMENT_OPEN_DATA_TEST_SESSION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        session_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.session")))
        session_ref01_data = None
        if len(session_ref01_data_raw) > 0:
...
def _session_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PARLAMENT_OPEN_DATA_TEST_SESSION_ENTID"))
