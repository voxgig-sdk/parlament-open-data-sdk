...
class TestSessionDirect:
    ...
    def test_should_direct_list_session(self):
        setup = _session_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-session", "live" if setup["live"] else "unit")
        if _skip:
...
def _session_direct_setup(mockres):
