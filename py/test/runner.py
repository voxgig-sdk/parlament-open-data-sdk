# ParlamentOpenData SDK test runner
...
class ParlamentOpenDataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ParlamentOpenDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ParlamentOpenDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ParlamentOpenDataTestRunner.getenv("PARLAMENT_OPEN_DATA_TEST_LIVE")
        override = ParlamentOpenDataTestRunner.getenv("PARLAMENT_OPEN_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ParlamentOpenDataTestRunner.getenv(key)
        ...
        explain = ParlamentOpenDataTestRunner.getenv("PARLAMENT_OPEN_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ParlamentOpenDataTestRunner._test_control is not None:
            return ParlamentOpenDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ParlamentOpenDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ParlamentOpenDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ParlamentOpenDataTestRunner.load_test_control()
...
def load_env_local():
    ParlamentOpenDataTestRunner.load_env_local()


def env_override(m):
    return ParlamentOpenDataTestRunner.env_override(m)


def entity_data(v):
    return ParlamentOpenDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ParlamentOpenDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ParlamentOpenDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ParlamentOpenDataTestRunner.load_test_control()


def live_delay_ms():
    return ParlamentOpenDataTestRunner.live_delay_ms()
