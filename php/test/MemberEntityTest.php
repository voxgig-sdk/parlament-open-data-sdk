...
class MemberEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PARLAMENT_OPEN_DATA_TEST_MEMBER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $member_ref01_data = null;
        if (count($member_ref01_data_raw) > 0) {
...
function member_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("PARLAMENT_OPEN_DATA_TEST_MEMBER_ENTID");
