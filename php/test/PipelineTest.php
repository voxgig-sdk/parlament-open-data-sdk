...
// ParlamentOpenData SDK pipeline test
//
...
{
    private static function utility(): ParlamentOpenDataUtility
    {
        return new ParlamentOpenDataUtility();
    }
    ...
    private static function ctx(array $over = []): ParlamentOpenDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ParlamentOpenDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ParlamentOpenDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ParlamentOpenDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ParlamentOpenDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ParlamentOpenDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ParlamentOpenDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ParlamentOpenDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ParlamentOpenDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ParlamentOpenDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ParlamentOpenDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ParlamentOpenDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ParlamentOpenDataResponse([]),
            'result' => new ParlamentOpenDataResult([]),
        ]);
        [, $err] = ParlamentOpenDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ParlamentOpenDataSpec([]),
            'response' => null,
            'result' => new ParlamentOpenDataResult([]),
        ]);
        [, $err] = ParlamentOpenDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ParlamentOpenDataSpec(['step' => 's']),
            'response' => new ParlamentOpenDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ParlamentOpenDataResult([]),
        ]);
        [, $err] = ParlamentOpenDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ParlamentOpenDataSpec(['step' => 's']),
            'response' => new ParlamentOpenDataResponse(self::resp(200, ['v' => 1])),
            'result' => new ParlamentOpenDataResult([]),
        ]);
        [, $err] = ParlamentOpenDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ParlamentOpenDataSpec(['step' => 's']),
            'response' => new ParlamentOpenDataResponse(self::resp(200, ['v' => 2])),
            'result' => new ParlamentOpenDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ParlamentOpenDataMakeResponse::call($ctx);
    ...
    {
        $preset = new ParlamentOpenDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ParlamentOpenDataSpec([]),
            'response' => new ParlamentOpenDataResponse([]),
            'result' => new ParlamentOpenDataResult([]),
        ...
        [$got, $err] = ParlamentOpenDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ParlamentOpenDataResult([])]);
        [, $err] = ParlamentOpenDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ParlamentOpenDataSpec([]), 'result' => null]);
        [, $err] = ParlamentOpenDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ParlamentOpenDataSpec(['step' => 's']),
            'result' => new ParlamentOpenDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ParlamentOpenDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ParlamentOpenDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ParlamentOpenDataSpec(['step' => 's']),
            'result' => new ParlamentOpenDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ParlamentOpenDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ParlamentOpenDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ParlamentOpenDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ParlamentOpenDataSpec([]),
            'result' => new ParlamentOpenDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ParlamentOpenDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ParlamentOpenDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ParlamentOpenDataError('boom', 'boom');
        $utility->fetcher = function (ParlamentOpenDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ParlamentOpenDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ParlamentOpenDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ParlamentOpenDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ParlamentOpenDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ParlamentOpenDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ParlamentOpenDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ParlamentOpenDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ParlamentOpenDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ParlamentOpenDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ParlamentOpenDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ParlamentOpenDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ParlamentOpenDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ParlamentOpenDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ParlamentOpenDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ParlamentOpenDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ParlamentOpenDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ParlamentOpenDataResult(['ok' => false])]);
        $this->expectException(ParlamentOpenDataError::class);
        ParlamentOpenDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ParlamentOpenDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ParlamentOpenDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ParlamentOpenDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ParlamentOpenDataBaseFeature();
        $b = new ParlamentOpenDataBaseFeature();
        ParlamentOpenDataFeatureAdd::call($ctx, $a);
        ParlamentOpenDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ParlamentOpenDataBaseFeature
    {
        $f = new ParlamentOpenDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ParlamentOpenDataFeatureAdd::call($ctx, self::named_feature('a'));
        ParlamentOpenDataFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        ParlamentOpenDataFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        ParlamentOpenDataFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        ParlamentOpenDataFeatureAdd::call($ctx, $replace);
        ...
        ParlamentOpenDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ParlamentOpenDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ParlamentOpenDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ParlamentOpenDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ParlamentOpenDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ParlamentOpenDataResponse(['headers' => null]),
            'result' => new ParlamentOpenDataResult([]),
        ]);
        ParlamentOpenDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ParlamentOpenDataResultBody::call($ctx);
