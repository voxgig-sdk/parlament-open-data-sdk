...
// ParlamentOpenData SDK Business entity
...
class BusinessEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BusinessEntity($this->_client, $opts);
    }

    /**
     * @param Business|array $args Business data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ParlamentOpenDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Business|array The current Business data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Business fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ParlamentOpenDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Business fields).
     */
    ...
    /**
     * List Business items matching the given filter.
     *
     * @param BusinessListMatch|array|null $reqmatch Match filter (any subset
     *   of Business fields) as an assoc-array; BusinessListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Business[]|array A list of Business items as assoc-arrays at
     *   the SDK boundary; throws ParlamentOpenDataError on failure (item-5 convention).
     */
