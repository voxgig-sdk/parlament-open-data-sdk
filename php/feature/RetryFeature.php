...
// ParlamentOpenData SDK retry feature
...
class ParlamentOpenDataRetryFeature extends ParlamentOpenDataBaseFeature
{
    ...
    public function init(ParlamentOpenDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ParlamentOpenDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ParlamentOpenDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ParlamentOpenDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
