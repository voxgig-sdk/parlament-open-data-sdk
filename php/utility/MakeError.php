...
class ParlamentOpenDataMakeError
{
    public static function call(?ParlamentOpenDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ParlamentOpenDataContext([], null);
        }
        $op = $ctx->op ?? new ParlamentOpenDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ParlamentOpenDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ParlamentOpenDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ParlamentOpenDataError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof ParlamentOpenDataError) {
