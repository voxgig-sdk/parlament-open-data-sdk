...
class ParlamentOpenDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ParlamentOpenDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ParlamentOpenDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ($utility->feature_add)($this->_rootctx, ParlamentOpenDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ParlamentOpenDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ParlamentOpenDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ParlamentOpenDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ParlamentOpenDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ParlamentOpenDataError($op . "_allow",
                "ParlamentOpenDataSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ParlamentOpenDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ParlamentOpenDataError("graphql_error",
                "ParlamentOpenDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Business($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_business === null) {
                $this->_business = new BusinessEntity($this, null);
            }
            return $this->_business;
        }
        return new BusinessEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new ParlamentOpenDataSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
