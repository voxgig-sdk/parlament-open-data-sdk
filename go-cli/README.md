...
boru-driven command-line client **and** interactive REPL for the ParlamentOpenData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/parlament-open-data-cli)
make build

# 2. See usage (words, entities, env vars)
./parlament-open-data-cli --help
...
./parlament-open-data-cli list member

# 5. Override the API base URL for a single call
PARLAMENT_OPEN_DATA_BASE=https://api.example.com ./parlament-open-data-cli list business

# 6. No arguments -> interactive REPL
./parlament-open-data-cli
parlament-open-data> list business
parlament-open-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/parlament-open-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/parlament-open-data-cli list business
   ```
...
Run with no arguments to open a REPL (prompt `parlament-open-data>`). Each line is
evaluated as its own boru expression:

```text
$ ./parlament-open-data-cli
parlament-open-data> list business
parlament-open-data> /help
parlament-open-data> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `business`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/parlament-open-data-cli`. |
...
(`./parlament-open-data-cli <expr>`) and interactively (the REPL), and expressions compose the
