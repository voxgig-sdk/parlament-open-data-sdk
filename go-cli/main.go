// Command ParlamentOpenData-cli is a CLI + REPL for the ParlamentOpenData SDK,
...
const prompt = "parlament-open-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "business member session"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PARLAMENT_OPEN_DATA_APIKEY (api key), PARLAMENT_OPEN_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PARLAMENT_OPEN_DATA_APIKEY carries the API key and
	// PARLAMENT_OPEN_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PARLAMENT_OPEN_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PARLAMENT_OPEN_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewParlamentOpenDataSDK(opts)
