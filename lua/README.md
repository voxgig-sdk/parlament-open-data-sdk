# ParlamentOpenData Lua SDK



The Lua SDK for the ParlamentOpenData API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Business()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/parlament-open-data-sdk/releases)),
...
Create an instance: `local business = client:Business(nil)`
...
```
lua/
├── parlament-open-data_sdk.lua    -- Main SDK module
...
The main module (`parlament-open-data_sdk`) exports the SDK constructor
