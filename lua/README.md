...
The Lua SDK for the ParlamentOpenData API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Business(nil):list(nil, nil)
...
local result, err = client:ParlamentOpenData(nil):load(
  { id = "test01" }, nil
)
