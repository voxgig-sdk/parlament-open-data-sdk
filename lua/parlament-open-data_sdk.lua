...
require("parlament-open-data_types")
...
local ParlamentOpenDataSDK = {}
ParlamentOpenDataSDK.__index = ParlamentOpenDataSDK
...
ParlamentOpenDataSDK._make_feature = _make_feature


function ParlamentOpenDataSDK.new(options)
  local self = setmetatable({}, ParlamentOpenDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ParlamentOpenDataSDK:options_map()
...
function ParlamentOpenDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function ParlamentOpenDataSDK:get_root_ctx()
  return self._rootctx
end


function ParlamentOpenDataSDK:prepare(fetchargs)
...
function ParlamentOpenDataSDK:direct(fetchargs)
...
function ParlamentOpenDataSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ParlamentOpenDataSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ParlamentOpenDataSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ParlamentOpenDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ParlamentOpenDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ParlamentOpenDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ParlamentOpenDataSDK:Business(data)
  ...
  if data == nil then
    if self._business == nil then
      self._business = EntityMod.new(self, nil)
    end
...
function ParlamentOpenDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ParlamentOpenDataSDK.new(sdkopts)
  sdk.mode = "test"
