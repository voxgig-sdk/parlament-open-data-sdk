...
local SessionEntity = {}
SessionEntity.__index = SessionEntity


function SessionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SessionEntity)
  self._name = "session"
  self._client = client
...
function SessionEntity:get_name()
  return self._name
end


function SessionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SessionEntity.new(self._client, opts)
end
...
function SessionEntity:mark_deleted()
  self._deleted = true
end


function SessionEntity:deleted()
  return true == self._deleted
end


function SessionEntity:data_set(args)
  if args ~= nil then
...
function SessionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SessionEntity:match_set(args)
  if args ~= nil then
...
function SessionEntity:match_get()
...
function SessionEntity:stream(action, args, callopts)
...
function SessionEntity:_run_op(ctx, post_done)
  local utility = self._utility
