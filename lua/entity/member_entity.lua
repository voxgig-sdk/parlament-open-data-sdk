...
local MemberEntity = {}
MemberEntity.__index = MemberEntity


function MemberEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MemberEntity)
  self._name = "member"
  self._client = client
...
function MemberEntity:get_name()
  return self._name
end


function MemberEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MemberEntity.new(self._client, opts)
end
...
function MemberEntity:mark_deleted()
  self._deleted = true
end


function MemberEntity:deleted()
  return true == self._deleted
end


function MemberEntity:data_set(args)
  if args ~= nil then
...
function MemberEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MemberEntity:match_set(args)
  if args ~= nil then
...
function MemberEntity:match_get()
...
function MemberEntity:stream(action, args, callopts)
...
---@param reqmatch MemberListMatch
---@param ctrl? table
---@return Member[]
---@return string? err
function MemberEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function MemberEntity:_run_op(ctx, post_done)
  local utility = self._utility
